function binned_stellar_sed, stellar_models, psi, atten_curve=atten_curve, tauV_diff=tauV_diff, $
                             ...
                             uv_bump=uv_bump, error_check=error_check, steps_mean_Lnu=steps_mean_Lnu, $
...
; Generate the attenuation in terms of exp(-tau)
 ...
 steps_mean_Lnu_red = dblarr(Nfilters, Nsteps, Nmodels)
...
; Calculate the mean Lnu of each filter for each model and bolometric luminosity of absorbed emission
 for st=0,(Nsteps-1) do begin
   ...
   case strupcase(atten_curve) of
     'CALZETTI00': steps_Lnu_red = exp_neg_tau * steps_Lnu_unred

     'CALZETTI_MOD': begin
          if st eq 0 then steps_Lnu_red = exp_neg_tau_BC * steps_Lnu_unred else $
                          steps_Lnu_red = exp_neg_tau * steps_Lnu_unred
        end

     'DOORE21': begin
          if rold0_ages[st] eq 0 then steps_Lnu_red = exp_neg_tau_young * steps_Lnu_unred
          if rold0_ages[st] eq 1 then steps_Lnu_red = exp_neg_tau_old * steps_Lnu_unred
        end
   endcase
   ...
   for nm=0,(Nmodels-1) do begin
     for f=0,(Nfilters-1) do begin
       ...
       steps_mean_Lnu_red[f, st, nm] = -1.d0*trapez(steps_Lnu_red[*, nm] * transmission[*, f], nu_obs)
     endfor
     if strupcase(atten_curve) ne 'DOORE21' then begin
       ; gives negative value from integral bc nu_obs is reversed (big to small)
       steps_Lbol_abs[st, nm] = Lbol + trapez(steps_Lnu_red[*, nm], nu_obs)
     endif
   endfor
 endfor
 ...
 steps_mean_Lnu = temporary(steps_mean_Lnu_red)
 mean_Lnu_stellar = total(steps_mean_Lnu * rebin(reform(psi, 1, Nsteps, Nmodels), Nfilters, Nsteps, Nmodels), 2, /NaN)
 mean_Lnu_unred_stellar = total(rebin(reform(stellar_models.mean_Lnu, Nfilters, Nsteps, 1), Nfilters, Nsteps, Nmodels) * $
                                rebin(reform(psi, 1, Nsteps, Nmodels), Nfilters, Nsteps, Nmodels), 2, /NaN)
