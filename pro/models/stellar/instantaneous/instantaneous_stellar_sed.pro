function instantaneous_stellar_sed, stellar_models, psi, atten_curve=atten_curve, tauV_diff=tauV_diff, $
                                    ...
                                    uv_bump=uv_bump, error_check=error_check, instant_mean_Lnu=instant_mean_Lnu, $
                                    mean_Lnu_unred_stellar=mean_Lnu_unred_stellar, Lbol_abs_stellar=Lbol_abs_stellar, $
                                    instant_Lbol_abs=instant_Lbol_abs
;+
; Name
; ----
;   INSTANTANEOUS_STELLAR_SED
;
; Purpose
; -------
;   Generates the attenuated stellar model SEDs for the instantaneous burst SFHs.
;   The output model SEDs depend on the filters, burst ages, and attenuation.
;
; Calling Sequence
; ----------------
;   ::
;
;       mean_Lnu_stellar = instantaneous_stellar_sed(stellar_models, psi [, atten_curve = , $
;                             tauV_diff = , delta = , tauV_BC = , tauB_f = , F_clump = , $
;                             cosi = , b_to_d = , rold0_ages = ,  atten_models = , $
;                             /uv_bump, /error_check, instant_mean_Lnu=instant_mean_Lnu, $
;                             mean_Lnu_unred_stellar=mean_Lnu_unred_stellar, $
;                             Lbol_abs_stellar=Lbol_abs_stellar, instant_Lbol_abs=instant_Lbol_abs])
;
; Inputs
; ------
;   ``stellar_models`` : structure
;       A structure containing the spectra, SEDs, and stellar parameters for the 
;       instantaneous stellar model. (See ``instantaneous_stellar_models.pro`` for
;       details and contents.)
;   ``psi`` : int, float, or double array(Nbursts, Nmodels)
;       The instantaneous burst SFH coefficients :math:`[M_\odot\ {\rm yr}^{-1}]`.
;
; Optional Inputs
; ---------------
...
;   ``rold0_ages`` : int, float, or double array(Nbursts)
;       The binary parameter ``rold0``, designating each SFH age burst as part of
...
; Optional Outputs
; ----------------
;   ``instant_mean_Lnu`` : double array(Nfilters, Nbursts, Nmodels)
;       The mean luminosity of each filter for each SFH age burst and set of stellar model 
...
;   ``instant_Lbol_abs`` : double array(Nbursts, Nmodels)
;       The bolometric luminosity of the attenuated stellar emission for
;       each SFH age burst and set of stellar model parameters :math:`[L_\odot]`.
;
; Note
; ----
...
; --------------------
;   - 2023/02/28: Created (Keith Doore)
;-
 On_error, 2
 compile_opt idl2
...
; Error handling
   ...
   case strupcase(atten_curve) of
     ...
     'DOORE21': begin
          ...
          if n_elements(rold0_ages) ne 0 then begin
            ...
            if n_elements(rold0_ages) ne Nbursts then $
              message, 'ROLD0_AGES must contain the same number of elements as the first dimension of PSI.'
            if total(rold0_ages eq 0 or rold0_ages eq 1) ne Nbursts then message, 'ROLD0_AGES must only contain values of 0 or 1.'
          endif else begin
            rold0_ages = dblarr(Nbursts)
 ...
 Nfilters = n_elements(stellar_models.FILTER_LABELS)
 Nbursts = n_elements(stellar_models.ages)
...
; Generate the attenuation in terms of exp(-tau)
 ...
 instant_mean_Lnu_red = dblarr(Nfilters, Nbursts, Nmodels)
 transmission = transpose(stellar_models.filters)
 instant_Lbol_abs = dblarr(Nbursts, Nmodels)

; Calculate the mean Lnu of each filter for each model and bolometric luminosity of absorbed emission
 for bs=0,(Nbursts-1) do begin
   instant_Lnu_unred = rebin(stellar_models.lnu[*, bs], nwave, Nmodels)

   ; Attenuate stellar models
   case strupcase(atten_curve) of
     'CALZETTI00': instant_Lnu_red = exp_neg_tau * instant_Lnu_unred

     'CALZETTI_MOD': begin
          if bs eq 0 then instant_Lnu_red = exp_neg_tau_BC * instant_Lnu_unred else $
                          instant_Lnu_red = exp_neg_tau * instant_Lnu_unred
        end

     'DOORE21': begin
          if rold0_ages[bs] eq 0 then instant_Lnu_red = exp_neg_tau_young * instant_Lnu_unred
          if rold0_ages[bs] eq 1 then instant_Lnu_red = exp_neg_tau_old * instant_Lnu_unred
        end
   endcase

   ;Lbol = -1.d0*trapez(instant_Lnu_unred, nu_obs)   ; does not include nebular emission absorption
   Lbol = (stellar_models.Lbol[bs])[0]
   for nm=0,(Nmodels-1) do begin
     for f=0,(Nfilters-1) do begin
       ...
       instant_mean_Lnu_red[f, bs, nm] = -1.d0*trapez(instant_Lnu_red[*, nm] * transmission[*, f], nu_obs)
     endfor
     if strupcase(atten_curve) ne 'DOORE21' then begin
       ; gives negative value from integral bc nu_obs is reversed (big to small)
       instant_Lbol_abs[bs, nm] = Lbol + trapez(instant_Lnu_red[*, nm], nu_obs)
     endif
   endfor
 endfor

 if strupcase(atten_curve) eq 'DOORE21' then begin
   instant_Lbol_abs = doore21_interp_lbol_abs_table(tauB_f, F_clump, b_to_d, rold0_ages[0:(Nbursts-1)], atten_models.doore21_Lbol_abs_table)
 endif

 ; Exclude NaNs in case a filter included NaNs.
 instant_mean_Lnu = temporary(instant_mean_Lnu_red)
 mean_Lnu_stellar = total(instant_mean_Lnu * rebin(reform(psi, 1, Nbursts, Nmodels), Nfilters, Nbursts, Nmodels), 2, /NaN)
 mean_Lnu_unred_stellar = total(rebin(reform(stellar_models.mean_Lnu, Nfilters, Nbursts, 1), Nfilters, Nbursts, Nmodels) * $
                                rebin(reform(psi, 1, Nbursts, Nmodels), Nfilters, Nbursts, Nmodels), 2, /NaN)
 Lbol_abs_stellar = total(instant_Lbol_abs * psi, 1, /nan)


 return, mean_Lnu_stellar
