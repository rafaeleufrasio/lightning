function instantaneous_stellar_spectrum, stellar_models, psi, atten_curve=atten_curve, tauV_diff=tauV_diff, $
                                         ...
                                         uv_bump=uv_bump, error_check=error_check, instant_Lnu_spec=instant_Lnu_spec, $
                                         Lnu_spec_unred_stellar=Lnu_spec_unred_stellar, Lbol_abs_stellar=Lbol_abs_stellar, $
                                         instant_Lbol_abs=instant_Lbol_abs
;+
; Name
; ----
;   INSTANTANEOUS_STELLAR_SPECTRUM
;
; Purpose
; -------
;   Generates the attenuated stellar model spectra for the instantaneous burst SFHs.
;   The output model spectra depend on the burst ages and attenuation.
;
; Calling Sequence
; ----------------
;   ::
;
;       Lnu_spec_stellar = instantaneous_stellar_spectrum(stellar_models, psi [, atten_curve = , $
;                                       tauV_diff = , delta = , tauV_BC = , tauB_f = , F_clump = , $
;                                       cosi = , b_to_d = , rold0_ages = , atten_models = , /uv_bump, $
;                                       /error_check, instant_Lnu_spec=instant_Lnu_spec, $
...
; Optional Outputs
; ----------------
;   ``instant_Lnu_spec`` : double array(Nwave, Nbursts, Nmodels)
;       The luminosity spectrum for each SFH age burst and set of stellar model 
...
; --------------------
;   - 2022/02/28: Created (Keith Doore)
;-
 On_error, 2
 compile_opt idl2
...
; Generate the attenuation in terms of exp(-tau)
 ...
 instant_Lnu_red = dblarr(Nwave, Nbursts, Nmodels)
...
; Calculate the mean Lnu of each filter for each model and bolometric luminosity of absorbed emission
 for bs=0,(Nbursts-1) do begin
   ...
   case strupcase(atten_curve) of
     'CALZETTI00': instant_Lnu_red[*, bs, *] = exp_neg_tau * instant_Lnu_unred

     'CALZETTI_MOD': begin
          if bs eq 0 then instant_Lnu_red[*, bs, *] = exp_neg_tau_BC * instant_Lnu_unred else $
                          instant_Lnu_red[*, bs, *] = exp_neg_tau * instant_Lnu_unred
        end

     'DOORE21': begin
          if rold0_ages[bs] eq 0 then instant_Lnu_red[*, bs, *] = exp_neg_tau_young * instant_Lnu_unred
          if rold0_ages[bs] eq 1 then instant_Lnu_red[*, bs, *] = exp_neg_tau_old * instant_Lnu_unred
        end
   endcase
   ...
   for nm=0,(Nmodels-1) do begin
     if strupcase(atten_curve) ne 'DOORE21' then begin
       ...
       instant_Lbol_abs[bs, nm] = Lbol + trapez(instant_Lnu_red[*, bs, nm], nu_obs)
     endif
   endfor
 endfor
 ...
 instant_Lnu_spec = temporary(instant_Lnu_red)
 Lnu_stellar_spec = total(instant_Lnu_spec * rebin(reform(psi, 1, Nbursts, Nmodels), Nwave, Nbursts, Nmodels), 2, /nan)
 Lnu_spec_unred_stellar = total(rebin(reform(stellar_models.Lnu, Nwave, Nbursts, 1), Nwave, Nbursts, Nmodels) * $
                                rebin(reform(psi, 1, Nbursts, Nmodels), Nwave, Nbursts, Nmodels), 2, /NaN)
