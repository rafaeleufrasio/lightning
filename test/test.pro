...
;coeff_guess = [2.0d,2.0d,2.0d,2.0d,2.0d] & coeff_scale = coeff_guess*0.1
coeff_guess = coeff_tru & coeff_scale = coeff_guess*0.1
...
par1_guess = 0.4d  &  par1_scale = 0.05d  &  k1_guess = interpol(lindgen(n1),tauV_DIFF_grid,par1_guess)
par2_guess = 0.0d  &  par2_scale = 0.05d  &  k2_guess = interpol(lindgen(n2),delta_grid,    par2_guess)
par3_guess = 0.4d  &  par3_scale = 0.05d  &  k3_guess = interpol(lindgen(n3),tauV_BC_grid,  par3_guess)
...
chain_old = [coeff_old,par1_old,par2_old,par3_old]
scale = [coeff_scale,par1_scale,par2_scale,par3_scale]
chain = [chain_old]
chi2_chain = [chi2_old]
...
for trial=0,Ntrials-1 do begin                                $
  ...
  coeff_new = chain_new[0:4]                                 &$
  psi1_new = chain_new[0] & while (psi1_new lt 0) do psi1_new = chain_old[0] + scale[0]*randomn(seed)  &$
  psi2_new = chain_new[1] & while (psi2_new lt 0) do psi2_new = chain_old[1] + scale[1]*randomn(seed)  &$
  psi3_new = chain_new[2] & while (psi3_new lt 0) do psi3_new = chain_old[2] + scale[2]*randomn(seed)  &$
  psi4_new = chain_new[3] & while (psi4_new lt 0) do psi4_new = chain_old[3] + scale[3]*randomn(seed)  &$
  psi5_new = chain_new[4] & while (psi5_new lt 0) do psi5_new = chain_old[4] + scale[4]*randomn(seed)  &$
  coeff_new = [psi1_new,psi2_new,psi3_new,psi4_new,psi5_new] &$
  ...
  chain_new = [coeff_new,par1_new,par2_new,par3_new]         &$
  Lmod_new = total(models[*,*,k1_new,k2_new,k3_new] * (replicate(1.d0,Nfilters+1)#coeff_new),2) &$
...
Mstar_chain = total(chain[0:4,*] * (steps.Mstar # replicate(1.d0,Nchain)),1)
SFR100_chain = 0.1*reform(chain[0,*]) + 0.9*reform(chain[1,*])
