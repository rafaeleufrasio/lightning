...
function dl07_sed,dl07,alpha=alpha,umin=umin,umax=umax,gam=gam,modeltype=modeltype,qPAH=qPAH,filter_labels=filter_labels,$
...
if n_elements(alpha) eq 0 then alpha = 2.0   else alpha = (alpha)[0]
if n_elements(Umin)  eq 0 then Umin  = 0.1   else Umin  = (Umin)[0]
if n_elements(Umax)  eq 0 then Umax  = 1.e5  else Umax  = (Umax)[0]
if n_elements(gam)   eq 0 then gam   = 1.0   else gam   = (gam)[0]
if n_elements(qPAH)  eq 0 then qPAH  = 0.025 else qPAH  = (qPAH)[0]
...
Lbol = dblarr(N_u)
mean_Lnu = dblarr(N_u,Nfilters)
for uu=0,(N_u-1) do begin 
  Lbol[uu] = interpol(reform((dl07.Lbol)[uu,*]),qPAH_vec,qPAH) ;Lbol[uu]
...
Lpow = gam*trap_int(U,Lbol*fU,xrange=[Umin,Umax],/pow)

mean_Lnu_pow = dblarr(Nfilters)
for k=0,(Nfilters-1) do mean_Lnu_pow[k] = gam*trap_int(U,mean_Lnu[*,k]*fU,xrange=[Umin,Umax],/pow)
mean_Lnu_pow[where(mean_Lnu_pow lt 1.d-150,/null)] = 0.d0


;delta function at Umin
Ldel = (1.d0-gam)*interpol(Lbol,U,Umin)

mean_Lnu_del = dblarr(Nfilters)
for k=0,(Nfilters-1) do mean_Lnu_del[k] = (1.d0-gam)*interpol(mean_Lnu[*,k],U,Umin)
...
function dl07_spec,dl07,alpha=alpha,umin=umin,umax=umax,gam=gam,qPAH=qPAH,modeltype=modeltype,$
...
;   added gamma and del component, Sept 30 2020, Keith Doore and Rafael Eufrasio
  ...
  Lnu_pow = dblarr(Nwave)
  for kk=0,(Nwave-1) do Lnu_pow[kk] = gam*trap_int(U,Lnu[*,kk]*fU,xrange=[Umin,Umax],/pow)

  ;delta function at Umin
  Ldel = (1.d0-gam)*interpol(Lbol,U,Umin)
  
  Lnu_del = dblarr(Nwave)
  for kk=0,(Nwave-1) do Lnu_del[kk] = (1.d0-gam)*interpol(Lnu[*,kk],U,Umin)
